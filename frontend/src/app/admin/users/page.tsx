...
function UserAvatar({ name, isActive }: { name: string; isActive?: boolean }) {
    const colors = ["bg-blue-400", "bg-purple-400", "bg-pink-400", "bg-orange-400", "bg-teal-400"];
    const color = colors[name.charCodeAt(0) % colors.length];
    return (
        <div className="relative shrink-0">
            <div className={`flex size-9 items-center justify-center rounded-full ${color} text-sm font-bold text-white`}>
                {name[0]?.toUpperCase()}
            </div>
            <span
                className={`absolute -bottom-0.5 -right-0.5 size-2.5 rounded-full ring-2 ring-white ${isActive !== false ? "bg-green-400" : "bg-gray-300"
                    }`}
            />
        </div>
    );
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className="flex size-10 items-center justify-center rounded-xl bg-primary/10">
                            <MaterialIcon name="person_add" filled className="text-primary text-[20px]" />
                        </div>
                        <h2 className="text-lg font-black text-text-main">Add New User</h2>
                    </div>
                    <button onClick={onClose} className="flex size-8 items-center justify-center rounded-full hover:bg-gray-100 text-gray-400">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <button type="submit" disabled={loading}
                            ...
                            {loading && <span className="size-4 animate-spin rounded-full border-2 border-white border-t-transparent" />}
                            Create User
                        </button>
                    </div>
                </form>
...
const DEMO_STATUSES: UserStatus[] = ["Active", "Payment Failed", "Suspended", "Active", "Expiring Soon"];
const DEMO_SUBS = [
...
export default function AdminUsersPage() {
    ...
    const fetchUsers = useCallback(async (page: number) => {
        ...
        try {
            ...
            setTotalCount(0);
            setTotalPages(1);
            setError("User list endpoint is not yet available on the backend. Add GET /api/management/users to retrieve real data.");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    return (
        <AdminShell activePage="users" pageTitle="User Management">
            ...
            <div className="mb-6 grid grid-cols-2 gap-4 xl:grid-cols-4">
                {STAT_CARDS.map((card) => (
                    <div key={card.label} className="flex items-start justify-between rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
                        <div>
                            <p className="text-xs font-semibold text-gray-400">{card.label}</p>
                            <p className="mt-1 text-2xl font-black text-text-main">{card.value}</p>
                        ...
                        <div className={`flex size-10 shrink-0 items-center justify-center rounded-xl ${card.iconBg}`}>
                            <MaterialIcon name={card.icon} filled className={`text-[22px] ${card.iconColor}`} />
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-6 py-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div className="relative">
                            ...
                            >
                                {["All Plans", "Yearly Pro", "Monthly Basic", "Free Tier", "Staff Access"].map((p) => (
                                    <option key={p}>{p}</option>
                                ))}
                            </select>
                            <MaterialIcon name="keyboard_arrow_down" className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-[18px] text-gray-400" />
                        </div>
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="h-9 appearance-none rounded-xl border border-gray-200 bg-white pl-3 pr-8 text-sm font-semibold text-text-muted outline-none focus:border-primary"
                            >
                                {["All Status", "Active", "Suspended", "Payment Failed", "Expiring Soon"].map((s) => (
                ...
                {error && (
                    <div className="mx-6 my-4 flex items-start gap-2 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-700">
                        <MaterialIcon name="warning" filled className="mt-0.5 shrink-0 text-[16px]" />
                        <span>{error}</span>
                    </div>
                )}
                ...
                <table className="w-full border-collapse text-sm">
                    <thead>
                        <tr className="border-b border-gray-100">
                            {["User", "Role", "Subscription", "Amount", "Status", "Last Login", "Actions"].map((h) => (
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            [
                                { name: "Alex Johnson", email: "alex.j@example.com", role: "Member" as UserRole, sub: DEMO_SUBS[0], status: "Active" as UserStatus, last: "2 mins ago" },
                                { name: "Sarah Smith", email: "sarah.s@example.com", role: "Contributor" as UserRole, sub: DEMO_SUBS[1], status: "Payment Failed" as UserStatus, last: "5 hours ago" },
                                { name: "Michael Chen", email: "m.chen@example.com", role: "Member" as UserRole, sub: DEMO_SUBS[2], status: "Suspended" as UserStatus, last: "2 months ago" },
                                { name: "Emily Davis", email: "emily.d@example.com", role: "Editor" as UserRole, sub: DEMO_SUBS[3], status: "Active" as UserStatus, last: "1 day ago" },
                                { name: "Jessica Wong", email: "j.wong@example.com", role: "Member" as UserRole, sub: DEMO_SUBS[4], status: "Expiring Soon" as UserStatus, last: "3 days ago" },
                            ].map((row, i) => (
                                <tr key={i} className="group border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <UserAvatar name={row.name} isActive={row.status === "Active"} />
                                            <div>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`flex items-center gap-1.5 text-sm font-semibold ${ROLE_STYLES[row.role]}`}>
                                            <MaterialIcon name={ROLE_ICONS[row.role]} className="text-[16px]" />
                                            {row.role}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <p className="font-semibold text-text-main">{row.sub.plan}</p>
                                        {row.sub.billing && <p className="text-xs text-gray-400">Next billing: {row.sub.billing}</p>}
                                        {!row.sub.billing && row.sub.plan !== "Free Tier" && <p className="text-xs text-gray-400">—</p>}
                                    </td>
                                    <td className="px-6 py-4 font-semibold text-text-main">{row.sub.amount}</td>
                                    <td className="px-6 py-4"><StatusBadge status={row.status} /></td>
                                    <td className="px-6 py-4 text-sm text-gray-400">{row.last}</td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100">
                                            <button title="Edit" className="flex size-7 items-center justify-center rounded-lg border border-gray-200 text-gray-400 hover:border-primary/30 hover:bg-primary/5 hover:text-primary transition">
                                                <MaterialIcon name="edit" className="text-[14px]" />
                                            </button>
                                            <button title="Suspend" className="flex size-7 items-center justify-center rounded-lg border border-gray-200 text-gray-400 hover:border-yellow-200 hover:bg-yellow-50 hover:text-yellow-600 transition">
                                                <MaterialIcon name="block" className="text-[14px]" />
                                            </button>
                                            <button title="Delete" className="flex size-7 items-center justify-center rounded-lg border border-gray-200 text-gray-400 hover:border-red-200 hover:bg-red-50 hover:text-red-500 transition">
                            ...
                            filtered.map((user, idx) => {
                                ...
                                return (
                                    <tr key={user.id} className="group border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <UserAvatar name={user.fullName ?? user.username} isActive={user.isActive} />
                                                <div>
                                                    <p className="font-bold text-text-main">{user.fullName ?? user.username}</p>
                                                    <p className="text-xs text-gray-400">{user.email ?? "@" + user.username}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={`flex items-center gap-1.5 text-sm font-semibold ${ROLE_STYLES[role]}`}>
                                                <MaterialIcon name={ROLE_ICONS[role]} className="text-[16px]" />
                                                {role}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <p className="font-semibold text-text-main">{sub.plan}</p>
                                            {sub.billing && <p className="text-xs text-gray-400">Next billing: {sub.billing}</p>}
                                        </td>
                                        <td className="px-6 py-4 font-semibold text-text-main">{sub.amount}</td>
                                        <td className="px-6 py-4"><StatusBadge status={status} /></td>
                                        <td className="px-6 py-4 text-sm text-gray-400">{timeAgo(user.lastLogin ?? user.createdAt)}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100">
                ...
                <div className="flex items-center justify-between border-t border-gray-100 px-6 py-4">
                    <p className="text-sm text-text-muted">
                        ...
                        <span className="font-bold text-text-main">
                            {filtered.length === 0 ? "1 to 5" : `${(currentPage - 1) * PAGE_SIZE + 1} to ${Math.min(currentPage * PAGE_SIZE, totalCount)}`}
                        </span>{" "}
                        of{" "}
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            onClick={() => setCurrentPage((p) => Math.min(totalPages || 10, p + 1))}
            ...
            {showAddModal && (
                <AddUserModal
                    ...
                    onSaved={() => { setShowAddModal(false); fetchUsers(currentPage); }}
                />
            )}
        </AdminShell>
    );
